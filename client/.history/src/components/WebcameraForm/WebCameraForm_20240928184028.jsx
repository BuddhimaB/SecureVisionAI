...
const WebCameraForm = () => {
  ...
  const addCamera = async () => {
    ...
    const cameraData = {
      systemId: "1c2b187e-6ec2-4b6f-912b-04d6cbfea2b8", // Example system ID
      name: name,
      type: "WEB_CAMERA",
      status: "RUNNING",
    };
    ...
    try {
      ...
      if (response && (response.status === 200 || response.status === 201)) {
        ...
        localStorage.setItem('cameraId2', response.data.camera.id);
        ...
        console.log(cameracameraId);
    
        // Now attempt to add the camera to the local backend
        try {
          const localresponse = await api.locallyaddCamera(cameraDataForLocal);
