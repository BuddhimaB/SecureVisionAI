...
export const registerUser = createAsyncThunk(
    ...
    async (data, { rejectWithValue }) => {
      try {
        ...
        console.log("Response from backend:", response); // Debugging: Check backend response
        
        if (response?.data?.status === 201) {
          const user = response?.data?.data?.user;
          try {
            await api.local_user.sendUserDetails({
              id: user.id,
              email: user.email,
              firstName: user.firstName,
              lastName: user.lastName,
            ...
            console.error("Error saving local user details:", error);
          }
