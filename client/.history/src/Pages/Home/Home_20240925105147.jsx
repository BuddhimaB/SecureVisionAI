...
const Home = () => {
  ...
  useEffect(() => {
    // Fetch the cameraId initially
    const storedCameraId = localStorage.getItem('cameraId');
    if (storedCameraId) {
      setCameraId(storedCameraId);
      console.log("This is the camera that integrated");
      console.log(storedCameraId);
    }
  
    // Set an interval to continuously check for updates
    const intervalId = setInterval(() => {
      const updatedCameraId = localStorage.getItem('cameraId');
      if (updatedCameraId !== cameraId) {
        setCameraId(updatedCameraId);
        console.log("This is the camera that integrated");
        console.log(updatedCameraId);
      }
    }, 1000); // Check every second, adjust timing as necessary
