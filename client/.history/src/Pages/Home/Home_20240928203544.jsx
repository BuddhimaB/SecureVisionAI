...
const Home = () => {
  ...
  const [isCameraAdded, setIsCameraAdded] = useState(false); // Track if a camera has been added
  const navigate = useNavigate();

  // Function to update camera ID and trigger a re-render
  const handleCameraChange = (newCameraId) => {
    localStorage.setItem('cameraId', newCameraId);
    setCameraId(newCameraId);
    setIsCameraAdded(true); // Trigger a re-render after camera is added
  };

  // Simulating adding a camera for demonstration purposes
  useEffect(() => {
    const addNewCamera = () => {
      const updatedCameraId = '7fff9205-1bad-4c5e-bd3f-f55b58c7c8e9'; // Example camera ID
      handleCameraChange(updatedCameraId);
    };

    // Call this when a camera is added (replace this with your logic for adding cameras)
    addNewCamera();
  }, []);

  // Listen for cameraId updates (force re-render when cameraId changes)
  useEffect(() => {
    const updateCameraIdFromStorage = () => {
      const updatedCameraId = localStorage.getItem('cameraId');
      if (updatedCameraId && updatedCameraId !== cameraId) {
        setCameraId(updatedCameraId);
      }
    };

    // Check for updates in localStorage every second
    const intervalId = setInterval(() => {
      updateCameraIdFromStorage();
        ...
        <div className="right-section">
          ...
          <div className="right-section-bottom">
            ...
            <div className="user-images">
              ...
              <a href='/newuser'><img src='https://cdn.pixabay.com/photo/2017/08/16/00/29/add-person-2646097_960_720.png' alt='Add user' /></a>
            </div>
          </div>
        </div>
      </div>
