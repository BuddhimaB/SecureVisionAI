...
const Home = () => {
  ...
  const [cameraId, setCameraId] = useState(localStorage.getItem('cameraId2')); // Initialize state
  ...
  useEffect(() => {
    const updateCameraId = () => {
      const updatedCameraId = localStorage.getItem('cameraId2');
  ...
  return (
    <div>
      ...
      <p>Camera ID: {cameraId2}</p> {/* Display the current cameraId */}
      
      <img
        src={`${MODEL_API_HOST}/video_feed/81161848-5444-41e7-82d0-b3fe5dc1d82a`}
        width={380}
        height={240}
        alt="CCTV video"
        onError={(event) => {
