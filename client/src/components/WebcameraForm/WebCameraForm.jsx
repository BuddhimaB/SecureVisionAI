...
const WebCameraForm = () => {
  ...
  useEffect(() => {
    const fetchCameraData = async () => {
      if (!systemId || !authToken) {
        setError('Both Camera ID and token are required.');
        return;
      }
      try {
        const getcameradataresponse = await api.getCameras(systemId, authToken);
        console.log("Camera data retrieved:", getcameradataresponse);
      } catch (error) {
        console.error('Error getting camera data:', error);
      }
    };
  
    fetchCameraData(); // Call the async function
  }, [systemId, authToken]); // Add any dependencies here
  ...
  const addCamera = async () => {
    ...
    try {
      ...
      if (response && (response.status === 200 || response.status === 201)) {
        ...
        localStorage.setItem('SystemId',response.data.camera.systemId );
        localStorage.setItem('cameraStatus', response.data.camera.status);
        ...
        localStorage.setItem('cameraDataForLocal',cameraDataForLocal);
        console.log("cameraId is ",cameraId);
    
        // Now attempt to add the camera to the local backend
        try {
          console.log("Camera Data request payload", cameraDataForLocal);
